...
class CheckAIStudentParallelTab:
    """批量并发查询 AI Student 资格 Tab：接入 AccountSelectionPanel，支持并发。"""
    ...
    def __init__(self, parent, account_manager: AccountManager,
        ...
        ctk.CTkLabel(
            action_card,
            text="ℹ️ 功能：自动登录账号并查询 AI Student 活动资格",
        ...
        self.real_btn = ctk.CTkButton(
            btn_frame, text="▶ 执行并发查询", width=160, height=42,
            font=ctk.CTkFont(family="Segoe UI", size=14, weight="bold"),
            fg_color="#27ae60", hover_color="#219a52", corner_radius=8,
    ...
    def _on_start_parallel(self):
        ...
        if not messagebox.askyesno(
            "确认",
            f"即将为 {len(accounts)} 个 Google 账号并发查询 AI Student 资格。\n"
            f"并发数: {workers}\n"
        ...
        self.log_callback(f"━━━ 开始多并发查询 AI Student 资格：共 {len(accounts)} 个账号，并发数 {workers} ━━━")
    ...
    def _run_thread(self, accounts: list[dict], max_workers: int, headless: bool, keep_browser_open: bool):
        ...
        def run_one(idx: int, acc: dict) -> dict:
            ...
            # Stagger starts to avoid Google rate-limiting
            if idx > 0:
                time.sleep(idx * 2)
            ...
            return changer.login_and_check_ai_student(
                email=acc["email"],
        ...
        try:
            while True:
                ...
                if kind == "progress":
                    ...
                    self.status_callback(f"查询资格进度: {self._completed_count}/{self._total_count}")
                    self.log_callback(f"[查询资格 {idx+1}/{total}] {status}")

                elif kind == "item_done":
                    _, idx, total, email, ok, detail = msg
                    self._completed_count += 1
                    short = "有资格" if ok else "无资格"
    ...
    def _on_finished(self, results: list[dict]):
        ...
        self.log_callback(f"━━━ 多并发查询资格完成：有资格 {success}, 无资格 {failed} ━━━")
        ...
        self.progress_var.set(f"完成! 有资格 {success}, 无资格 {failed}")
        self.on_data_changed()
        self.selector.refresh()
        messagebox.showinfo("查询完成", f"有资格: {success}\n无资格: {failed}\n\n{summary[:500]}...")
