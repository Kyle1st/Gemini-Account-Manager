...
class FamilyParallelTab:
    """批量并发创建家庭组 Tab：接入 AccountSelectionPanel，支持并发创建家庭组。"""
    ...
    def __init__(self, parent, account_manager: AccountManager,
        ...
        # Share Google One & Keep Open options
        opt_frame2 = ctk.CTkFrame(config_card, fg_color="transparent")
        opt_frame2.pack(fill="x", padx=18, pady=(0, 14))

        self.share_google_one_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(
            opt_frame2,
            text="✅ 自动开启 Google One 共享",
            variable=self.share_google_one_var,
            font=ctk.CTkFont(size=13), checkbox_width=22, checkbox_height=22, corner_radius=4
        ...
        ctk.CTkLabel(
            action_card,
            text="ℹ️ 功能：自动登录账号并创建家庭组（如果已存在则跳过）",
        ...
        self.real_btn = ctk.CTkButton(
            btn_frame, text="▶ 执行并发创建", width=160, height=42,
    ...
    def _on_start_parallel(self):
        ...
        if not messagebox.askyesno(
            "确认",
            f"即将为 {len(accounts)} 个 Google 账号并发创建家庭组。\n"
            f"并发数: {workers}\n"
        ...
        self.log_callback(f"━━━ 开始多并发创建家庭组：共 {len(accounts)} 个账号，并发数 {workers} ━━━")

        threading.Thread(
            target=self._run_thread,
            args=(accounts, workers, self.headless_var.get(),
                  self.share_google_one_var.get(), self.keep_open_var.get()),
            daemon=True,
        ).start()
        self._check_queue()

    def _run_thread(self, accounts: list[dict], max_workers: int, headless: bool,
                    share_google_one: bool, keep_browser_open: bool):
        total = len(accounts)
        ...
        def run_one(idx: int, acc: dict) -> dict:
            ...
            return changer.create_family_group(
                email=acc["email"],
        ...
        try:
            while True:
                ...
                if kind == "progress":
                    ...
                    self.status_callback(f"创建家庭组进度: {self._completed_count}/{self._total_count}")
                    self.log_callback(f"[创建家庭组 {idx+1}/{total}] {status}")
    ...
    def _on_finished(self, results: list[dict]):
        ...
        self.log_callback(f"━━━ 多并发创建家庭组完成：成功 {success}, 失败 {failed} ━━━")
        ...
        messagebox.showinfo("多并发创建家庭组完成", f"成功: {success}\n失败: {failed}\n\n{summary[:500]}...")
