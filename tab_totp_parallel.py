...
class TotpParallelTab:
    """多并发改2FA Tab：接入 AccountSelectionPanel，支持并发重置 2FA。"""
    ...
    def __init__(self, parent, account_manager: AccountManager,
        ...
        self.real_btn = ctk.CTkButton(
            btn_frame, text="▶ 执行并发改 2FA", width=160, height=42,
            font=ctk.CTkFont(family="Segoe UI", size=14, weight="bold"),
            fg_color="#e74c3c", hover_color="#c0392b", corner_radius=8,
            command=self._on_real_totp_reset_parallel,
        )
        ...
        self.textbox.insert("1.0", "# 左侧勾选账号后，点击【▶ 执行并发改 2FA】开始操作\n# 程序会自动登录 Google 并重置 2FA\n# 新的 TOTP 密钥将自动保存到本地账号数据中\n")
    ...
    def _on_real_totp_reset_parallel(self):
        if self._running:
            return
        ...
        missing_totp = [acc["email"] for acc in accounts if not acc.get("totp_secret")]
        if missing_totp:
            msg = f"以下 {len(missing_totp)} 个账号没有 TOTP 密钥，可能无法登录:\n"
            msg += "\n".join(missing_totp[:5])
        ...
        if not messagebox.askyesno(
            "确认",
            f"即将并发重置 {len(accounts)} 个 Google 账号的 2FA。\n"
            f"并发数: {workers}\n"
        ...
        self.log_callback(f"━━━ 开始多并发改2FA：共 {len(accounts)} 个账号，并发数 {workers} ━━━")

        threading.Thread(
            target=self._run_thread,
            args=(accounts, workers, self.headless_var.get()),
            daemon=True,
        ).start()
        self._check_queue()

    def _run_thread(self, accounts: list[dict], max_workers: int, headless: bool):
        total = len(accounts)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            while futures:
                ...
                for fut in done:
                    ...
                    if result["success"] and result.get("new_totp_secret"):
                        self._queue.put(
        ...
        try:
            while True:
                ...
                if kind == "progress":
                    ...
                    self.log_callback(f"[并发改2FA {idx+1}/{total}] {email}: {short} {detail}")

                elif kind == "update_local":
                    _, email, new_secret = msg
    ...
    def _on_finished(self, results: list[dict]):
        ...
        self.log_callback(f"━━━ 多并发改2FA完成：成功 {success}, 失败 {failed} ━━━")

        self.textbox.delete("1.0", "end")
        all_accs = self.account_manager.get_all_accounts()
        email_to_acc = {acc["email"]: acc for acc in all_accs}
        lines = []
        for r in results:
            if r["success"] and r.get("new_totp_secret"):
                acc = email_to_acc.get(r["email"])
                if acc:
                     lines.append(AccountManager.format_line(acc))
        
        if lines:
            self.textbox.insert("1.0", "\n".join(lines))

        summary = "\n".join(
             f"{r['email']} - {'OK (新密钥 ' + r['new_totp_secret'][:8] + '...)' if r['success'] else 'FAIL: ' + r.get('message', '')}"
             for r in results
        )
        ...
        messagebox.showinfo("多并发改2FA完成", f"成功: {success}\n失败: {failed}\n\n{summary}")
